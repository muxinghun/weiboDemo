 ...
 * GET WEIBO USER INFO
 */
-(BOOL)getweiboUserInfo{
    if (![self isSessionValid] || ![self isOpenIdValid]) {
		return NO;
	}
	NSMutableDictionary * params = [[NSMutableDictionary alloc] init];

	APIBase *apiRequest = [[APIBase alloc] initWithTarget:self andSelector:@selector(getweiboUserInfoResponse:) andHttpMethod:@"GET"];
	NSString * fullURL = [kRestserverBaseURL stringByAppendingString:@"user/get_info"];
	[apiRequest openUrl:fullURL token:_accessToken openid:_openId appid:_appId params:params];
	[_apiRequests setObject:apiRequest forKey:[NSString stringWithFormat:@"%d", apiRequest.seq]];
	return YES;
...
-(BOOL)addweiboIdol{
	...
	NSMutableDictionary * params = [[NSMutableDictionary alloc] init];
    	[params setValue:@"linju2293579314" forKey:@"name"];
	APIBase *apiRequest = [[APIBase alloc] initWithTarget:self andSelector:@selector(addweiboIdolResponse:) andHttpMethod:@"POST"];
	NSString * fullURL = [kRestserverBaseURL stringByAppendingString:@"relation/add_idol"];
	[apiRequest openUrl:fullURL token:_accessToken openid:_openId appid:_appId params:params];
	[_apiRequests setObject:apiRequest forKey:[NSString stringWithFormat:@"%d", apiRequest.seq]];
	return YES;
 ...
 * Called when the get_user_info has response.
 */
- (void)getweiboUserInfoResponse:(APIResponse*) response
{
	if (!response)
		return;
	[self deleteAPIRequestBySeq:[NSString stringWithFormat:@"%d", response.seq]];
	if ([self.sessionDelegate respondsToSelector:@selector(getweiboUserInfoResponse:)]) 
	{
		[_sessionDelegate getweiboUserInfoResponse:response];
	}
}
/**
 * 微博关注
 */
- (void)addweiboIdolResponse:(APIResponse*) response
{
	if (!response)
		return;
	[self deleteAPIRequestBySeq:[NSString stringWithFormat:@"%d", response.seq]];
	if ([self.sessionDelegate respondsToSelector:@selector(addweiboIdolResponse:)]) 
	{
		[_sessionDelegate addweiboIdolResponse:response];
	}
}
