...
static NSString* kDefaultTitle = @"第三方应用授权";
...
static CGFloat kTitleMarginY = 9;
...
//																						  kCFStringEncodingUTF8);
            NSString* escaped_value = (NSString *)CFURLCreateStringByAddingPercentEscapes(
																						 NULL, (CFStringRef)value, NULL, CFSTR("￼=,!$&'()*+;@?\n\"<>#\t :/"), 
																						  kCFStringEncodingUTF8);
...
- (id)init {
	if (self = [super initWithFrame:CGRectZero]) {
		...
		CGFloat titleLabelFontSize = (TencentIsDeviceIPad() ? 22 : 18);
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request
 navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([_loginType isEqualToString:@"qqweibo"]) {
	...
	if (verifier && ![verifier isEqualToString:@""]) {
        ...
        if ([_delegate respondsToSelector:@selector(authorizeVerfyCode:didReceiveVerfyCode:)])
        {
            [_delegate authorizeVerfyCode:self didReceiveVerfyCode:verifier];
        }
...
//	NSLog([NSString stringWithFormat:@"absolute url : %@", [url absoluteURL]]);
	...
	else
	{
        ...
        if (range.location != NSNotFound)
        {
            ...
            if ([_delegate respondsToSelector:@selector(authorizeLoginView:didReceiveAuthorizeCode:)])
            {
                [_delegate authorizeLoginView:self didReceiveAuthorizeCode:code];
            }
            return NO;;
        }
         return YES;
